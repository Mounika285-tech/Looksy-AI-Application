...
export const WeatherStylingResultScreen = ({ navigation, route }) => {
  const { temp, cond } = route.params;
  const { user } = useAuth();
  ...
  // Forecast-specific style options
  ...
  const curateWeatherOutfit = async () => {
    try {
      setIsLoading(true);
      setIsSaved(false);
      ...
      console.error('Error curating weather outfit:', e);
  ...
  const createFallbackOutfit = () => {
    return {
      name: isWarm ? 'Chic Summer Breeze' : isCold ? 'Warm Cashmere Layering' : 'Soft Layers & Silk',
  ...
  const handleSaveOutfit = async () => {
    ...
    try {
      ...
      await set(newFavRef, {
        ...
        type: curatedOutfit.type || 'Weather Styling',
        createdAt: Date.now(),
        items: itemsData,
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loaderText}>LookSy AI is curating pieces for {temp}°C...</Text>
      </View>
    );
  }
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Weather Suggestion</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {/* Large Curated Outfit Hero Card */}
        <View style={styles.heroCard}>
          ...
          <View style={styles.heroTextContainer}>
            <View style={styles.badge}>
              <Text style={styles.badgeText}>Perfect for {temp}°C</Text>
            </View>
            <Text style={styles.heroTitleText}>{curatedOutfit.name}</Text>
            <Text style={styles.heroDesc}>
              Curated specifically for {cond} weather in your region to maintain ultimate comfort and premium elegance.
            </Text>
          </View>
        </View>
        ...
        <View style={styles.breakdownSection}>
          ...
          {/* Top */}
          {renderOutfitRow(curatedOutfit.items.top, 'The Base Layer', 'Top Wear', curatedOutfit.suggestedAdditions?.top)}

          {/* Bottom */}
          {renderOutfitRow(curatedOutfit.items.bottom, 'The Bottoms', 'Bottom Wear', curatedOutfit.suggestedAdditions?.bottom)}

          {/* Shoes */}
          {renderOutfitRow(curatedOutfit.items.footwear, 'The Footwear', 'Footwear', curatedOutfit.suggestedAdditions?.footwear)}

          {/* Outerwear */}
          {renderOutfitRow(curatedOutfit.items.accessory, 'Outerwear', 'Accessories', curatedOutfit.suggestedAdditions?.accessory)}
        </View>
